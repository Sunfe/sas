...
get_prev_date <- function(cur_date,date_offset,stock)
{
  ...
  res <- dbGetQuery(conn, sqlstr); 
  while(res[1,1] != (date_offset) & (prev_date >= start_trade_date))
  {
    i <- i + 1;
...
for(stock in g_stock_code)
{
  ...
  for(i in 1:max_calc_date_num)
  {
    ...
    if (!is_trade_date_exists(cur_date,stock))
      next;

    close_price <- get_cur_close_price(cur_date,stock);
...
time_start <- Sys.time();
sqlstr=paste0("select * from basic_stock_2010_2015");
res<-dbGetQuery(conn, sqlstr);
time_end <-Sys.time();
print(time_end - time_start)
